...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <x-ui.button type="button" id="openChallengeModalBtn" size="sm">
    ...
    <div class="space-y-4" id="challengesList">
        ...
        <div class="bg-white border border-gray-200 rounded-xl overflow-hidden"
            ...
            <div class="h-1
                ...
                @elseif($challenge->status === 'completed') bg-blue-500
                @elseif($challenge->status === 'analyzing' || in_array($challenge->ai_analysis_status, ['pending', 'processing'])) bg-violet-500
                @elseif($challenge->status === 'submitted') bg-amber-400
                @elseif($challenge->status === 'rejected') bg-red-500
                @else bg-gray-400
                @endif">
            </div>
            ...
            <div class="p-5">
                <div class="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-2 mb-3">
                            <a href="{{ route('volunteer.challenges.show', $challenge) }}"
                               class="text-base font-bold text-gray-900 hover:text-primary-600 truncate">
                                {{ $challenge->title }}
                            </a>

                            <!-- Status Badge -->
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold border
                                @if($challenge->status === 'active') bg-emerald-50 text-emerald-700 border-emerald-200
                                @elseif($challenge->status === 'completed') bg-blue-50 text-blue-700 border-blue-200
                                @elseif($challenge->status === 'analyzing') bg-violet-50 text-violet-700 border-violet-200
                                @elseif($challenge->status === 'submitted') bg-amber-50 text-amber-700 border-amber-200
                            ...
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">
                                <svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold border
                                @if($challenge->challenge_type === 'community_discussion') bg-cyan-50 text-cyan-700 border-cyan-200
                        ...
                        <!-- Description -->
                        <p class="text-gray-600 text-sm line-clamp-2 mb-3">{{ $challenge->original_description }}</p>

                        <!-- Rejection Message -->
                        @if($challenge->status === 'rejected' && $challenge->rejection_reason)
                        <div class="mb-3 bg-red-50 border border-red-200 rounded-lg px-3 py-2 flex items-start gap-2">
                            <svg class="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                            </svg>
                            <div>
                                <p class="text-xs font-semibold text-red-700">{{ __('Rejection Reason') }}</p>
                                <p class="text-xs text-red-600">{{ $challenge->rejection_reason }}</p>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <div class="flex items-center gap-1.5 bg-gray-50 px-2.5 py-1 rounded-lg border border-gray-200">
                                ...
                                <span class="text-xs font-bold text-amber-700">{{ $challenge->score }}/10</span>
                            </div>
                            @endif
                    ...
                    <div class="flex-shrink-0 flex items-center gap-2">
                        <x-ui.button as="a" href="{{ url('/my-challenges/' . $challenge->id) }}" variant="ghost" size="sm">
                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                            </svg>
                        ...
                        <x-ui.button as="a" href="{{ url('/my-challenges/' . $challenge->id) }}?action=edit" variant="ghost" size="sm">
                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <x-ui.button type="button" onclick="confirmDelete({{ $challenge->id }}, '{{ addslashes($challenge->title) }}')" variant="ghost" size="sm">
            ...
            <div class="px-5 pb-4">
                <div class="flex items-center justify-between text-xs text-gray-500 mb-1.5">
                    <span class="font-medium">{{ __('Progress') }}</span>
                    <span class="font-semibold text-primary-600">{{ $challenge->progress_percentage ?? 0 }}%</span>
                </div>
                <div class="w-full bg-gray-100 rounded-full h-2">
                    <div class="bg-primary-500 h-full rounded-full" style="width: {{ $challenge->progress_percentage ?? 0 }}%"></div>
                </div>
            </div>
            @endif
            ...
            <div class="px-5 py-3 bg-emerald-50 border-t border-emerald-100 flex items-center justify-between">
                <div class="flex items-center gap-2 text-emerald-700">
                    <svg class="w-4 h-4 text-emerald-600" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                    <span class="text-sm font-semibold">{{ __(':count quality solutions received', ['count' => $highScoreComments]) }}</span>
                </div>
    ...
    <div class="bg-white border border-dashed border-gray-300 rounded-xl p-12 text-center">
        ...
        <h3 class="text-lg font-bold text-gray-900 mb-1">{{ __('No Challenges Yet') }}</h3>
        <p class="text-gray-500 text-sm mb-6 max-w-md mx-auto">{{ __('You haven\'t submitted any challenges to the community yet. Start by sharing a problem you\'d like solved!') }}</p>
        <x-ui.button type="button" onclick="openChallengeModal()">
            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
            </svg>
...
<div id="challengeModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-gray-900/70 z-40" onclick="closeChallengeModal()"></div>
            ...
            <form id="challengeForm" enctype="multipart/form-data">
                ...
                <div class="bg-primary-500 px-6 py-4 border-b border-primary-400/30">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-white" id="modal-title">{{ __('Submit a Community Challenge') }}</h3>
                            <p class="text-white/80 text-xs mt-0.5">{{ __('Your challenge will be analyzed and scored by AI') }}</p>
                        </div>
                ...
                <div class="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="challenge_description" rows="5" required minlength="50"
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 text-sm"
                            placeholder="{{ __('Describe your challenge in detail...') }}"></textarea>
                        <div class="flex justify-between mt-1">
                            <p class="text-xs text-gray-500">{{ __('Minimum 50 characters') }}</p>
                            <p class="text-xs text-gray-500"><span id="charCount">0</span>/50</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="dropZone" class="border border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary-300 hover:bg-gray-50 cursor-pointer bg-gray-50">
                            ...
                            <label for="challenge_attachments" class="cursor-pointer">
                                <div class="w-12 h-12 mx-auto mb-3 bg-primary-100 rounded-xl flex items-center justify-center">
                                    <svg class="h-6 w-6 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                    </svg>
                                </div>
                                <p class="text-sm font-medium text-gray-700">{{ __('Click to upload or drag and drop') }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">{{ __('PDF, DOC, Images up to 10MB each') }}</p>
                            </label>
                        </div>
                    ...
                    <div class="bg-primary-50 border border-primary-200 rounded-lg p-4">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="font-semibold text-primary-900 text-sm mb-2">{{ __('How scoring works:') }}</p>
                                <div class="grid grid-cols-2 gap-2 text-xs">
                                    <div class="flex items-center gap-2">
                                        <span class="w-5 h-5 bg-cyan-100 text-cyan-700 rounded text-[10px] flex items-center justify-center font-bold">1-2</span>
                                        <span class="text-gray-600">{{ __('Community Discussion') }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="w-5 h-5 bg-indigo-100 text-indigo-700 rounded text-[10px] flex items-center justify-center font-bold">3-10</span>
                                        <span class="text-gray-600">{{ __('Team Execution') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-gray-50 px-6 py-4 flex items-center justify-end gap-2 border-t border-gray-100">
                    ...
                    <x-ui.button as="submit" id="submitBtn" size="sm">
                        <span id="submitText" class="flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/>
                            </svg>
                            {{ __('Submit Challenge') }}
                        </span>
                        <span id="submitLoading" class="hidden flex items-center gap-1.5">
...
<div id="deleteModal" class="fixed inset-0 z-50 hidden" aria-modal="true">
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4">
        <div class="relative bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-md p-6 text-center" onclick="event.stopPropagation()">
            ...
            <h3 class="text-lg font-bold text-gray-900 mb-1">{{ __('Delete Challenge?') }}</h3>
            <p class="text-gray-500 text-sm mb-1">{{ __('You\'re about to delete:') }}</p>
            <p class="text-gray-900 font-semibold mb-3 text-base" id="deleteTitle"></p>
            <p class="text-xs text-gray-400 mb-6">{{ __('This action cannot be undone. All comments and data will be permanently removed.') }}</p>
            <div class="flex justify-center gap-2">
                <x-ui.button type="button" onclick="closeDeleteModal()" variant="ghost" size="sm">
                    {{ __('Cancel') }}
                </x-ui.button>
                <x-ui.button type="button" id="deleteConfirmBtn" onclick="executeDelete()" variant="destructive" size="sm">
                    {{ __('Delete') }}
                </x-ui.button>
            </div>
        </div>
...
if (dropZone) {
    dropZone.addEventListener('dragover', (e) => {
        e.preventDefault();
        dropZone.classList.add('border-primary-400', 'bg-gray-100');
    });
    dropZone.addEventListener('dragleave', () => {
        dropZone.classList.remove('border-primary-400', 'bg-gray-100');
    });
...
document.querySelectorAll('.filter-chip').forEach(chip => {
    chip.addEventListener('click', function() {
        document.querySelectorAll('.filter-chip').forEach(c => {
            c.classList.remove('bg-primary-50', 'text-primary-700', 'border-primary-200');
            c.classList.add('bg-gray-50', 'text-gray-600', 'border-transparent');
        });
...
document.getElementById('sortSelect')?.addEventListener('change', function() {
    ...
    const cards = Array.from(list.querySelectorAll('[data-challenge-id]'));
