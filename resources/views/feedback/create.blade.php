...
@section('title', __('Submit Feedback'))
...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-6">
        <h1 class="text-xl font-bold text-slate-900 mb-1">{{ __('Submit Feedback') }}</h1>
        <p class="text-sm text-slate-500 mb-6">{{ __('Report a problem, share an idea, or request a feature.') }}</p>
        ...
        <form action="{{ route('feedback.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-2">
                    ...
                    <label class="flex items-center justify-center px-3 py-2.5 rounded-xl border cursor-pointer text-sm font-medium has-[:checked]:border-primary-500 has-[:checked]:bg-primary-50 has-[:checked]:text-primary-700 border-slate-200 text-slate-600">
                        <input type="radio" name="type" value="{{ $type }}" class="sr-only" {{ old('type') === $type ? 'checked' : '' }} required>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" maxlength="150" required
                       ...
                       placeholder="{{ __('Short, clear summary') }}">
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="6" minlength="20" maxlength="3000" required
                          ...
                          placeholder="{{ __('What\'s the problem, idea, or feature? The more detail, the better.') }}">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" name="category" id="category" value="{{ old('category') }}" maxlength="100"
                       class="w-full px-3.5 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500"
                       placeholder="{{ __('e.g. Dashboard, Matching, Certificates') }}">
            </div>

            <div class="flex justify-end gap-3 pt-2">
                <x-ui.button as="a" href="{{ route('feedback.index') }}" variant="secondary">{{ __('Cancel') }}</x-ui.button>
                <x-ui.button type="submit" variant="primary">{{ __('Post Feedback') }}</x-ui.button>
            </div>
        </form>
    </div>
</div>
