    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center">
            <div class="inline-flex items-center gap-2 bg-white/[0.06] border border-white/10 rounded-full px-4 py-1.5 mb-6 backdrop-blur-sm">
                ...
                <span class="text-sm font-medium text-gray-300">{{ __('Integrations') }}</span>
            </div>

            <h1 class="text-display text-gray-900 dark:text-white mb-5">
                {{ __('Mindova Connects to the') }} <span class="text-gradient-aurora">{{ __('Tools You Already Use') }}</span>
            </h1>
...
<section class="py-16">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 rounded-xl p-6 flex items-start gap-4 transition-premium hover:elevation-sm dark:hover:bg-white/[0.05]">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('LinkedIn') }}</h3>
                    <p class="text-gray-600 dark:text-white/60 text-sm">{{ __('Sign in or register with your LinkedIn account — no separate password to remember, and your professional details can pre-fill your contributor profile.') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 rounded-xl p-6 flex items-start gap-4 transition-premium hover:elevation-sm dark:hover:bg-white/[0.05]">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('WhatsApp') }}</h3>
                    <p class="text-gray-600 dark:text-white/60 text-sm">{{ __('Opt in to receive task invitations, status updates, and submission notifications on WhatsApp via the official Cloud API — no extra app to install.') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 rounded-xl p-6 flex items-start gap-4 transition-premium hover:elevation-sm dark:hover:bg-white/[0.05]">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('Claude AI') }}</h3>
                    <p class="text-gray-600 dark:text-white/60 text-sm">{{ __('Every challenge analysis, task decomposition, volunteer match, and quality score is generated by Anthropic\'s Claude models — the engine behind the entire AI pipeline.') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-white/[0.03] border border-gray-200 dark:border-white/10 rounded-xl p-6 flex items-start gap-4 transition-premium hover:elevation-sm dark:hover:bg-white/[0.05]">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-1">{{ __('SMS & Voice') }}</h3>
                    <p class="text-gray-600 dark:text-white/60 text-sm">{{ __('Twilio powers fallback SMS delivery for time-sensitive notifications when a contributor isn\'t reachable on WhatsApp.') }}</p>
                </div>
            </div>
        </div>
    </div>
...
<section class="py-16 bg-gray-50 dark:bg-transparent">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-4">{{ __('Want to See a Specific Integration?') }}</h2>
