...
<div x-data="inviteForm()" class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="relative overflow-hidden bg-primary-500 rounded-3xl p-6 lg:p-8 slide-up slide-up-1">
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
            ...
            <div class="absolute -bottom-20 -left-20 w-80 h-80 bg-indigo-400/20 rounded-full blur-3xl float-anim"></div>
        </div>

        <div class="relative z-10 flex items-center gap-4">
            <a href="{{ route('mindova.team.index') }}" class="p-3 bg-white/20 backdrop-blur-sm hover:bg-white/30 rounded-xl">
        ...
        <div class="relative z-10 mt-6 flex items-center justify-center gap-4">
            <div class="flex items-center gap-2">
                <div :class="step >= 1 ? 'bg-white text-indigo-600' : 'bg-white/20 text-white'" class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">1</div>
                <span class="text-white/80 text-sm hidden sm:inline">{{ __('Details') }}</span>
            </div>
            <div class="w-12 h-0.5 bg-white/30"></div>
            <div class="flex items-center gap-2">
                <div :class="step >= 2 ? 'bg-white text-indigo-600' : 'bg-white/20 text-white'" class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">2</div>
                <span class="text-white/80 text-sm hidden sm:inline">{{ __('Role') }}</span>
            </div>
            <div class="w-12 h-0.5 bg-white/30"></div>
            <div class="flex items-center gap-2">
                <div :class="step >= 3 ? 'bg-white text-indigo-600' : 'bg-white/20 text-white'" class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">3</div>
    ...
    <form action="{{ route('mindova.team.store') }}" method="POST" @submit="validateForm($event)">
        ...
        <div x-show="step === 1"    class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden slide-up slide-up-2">
            ...
            <div class="p-6 space-y-6">
                <div class="input-field">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="name" name="name" x-model="form.name" required
                            class="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white @error('name') border-red-500 bg-red-50 @enderror"
                ...
                <div class="input-field">
                    ...
                    <div class="relative">
                        ...
                        <input type="email" id="email" name="email" x-model="form.email" required
                            class="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:bg-white @error('email') border-red-500 bg-red-50 @enderror"
                ...
                <div class="flex justify-end pt-4 border-t border-slate-100">
                    <x-ui.button @click="nextStep()" x-bind:disabled="!form.name || !form.email" variant="primary">
                        {{ __('Continue') }}
        ...
        <!-- Step 2: Select Role -->
        <div x-show="step === 2" x-cloak    class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-5 border-b border-slate-100 bg-white">
                <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    @php
                        $roleColors = [
                            'owner' => ['border' => 'border-amber-300', 'bg' => 'bg-gray-50', 'icon' => 'bg-secondary-400', 'badge' => 'bg-amber-100 text-amber-700'],
                            'admin' => ['border' => 'border-purple-300', 'bg' => 'bg-secondary-50', 'icon' => 'bg-secondary-500', 'badge' => 'bg-purple-100 text-purple-700'],
                            'accounting' => ['border' => 'border-emerald-300', 'bg' => 'bg-gray-50', 'icon' => 'bg-secondary-500', 'badge' => 'bg-emerald-100 text-emerald-700'],
                            'support' => ['border' => 'border-blue-300', 'bg' => 'bg-gray-50', 'icon' => 'bg-primary-500', 'badge' => 'bg-blue-100 text-blue-700'],
                            'feedback-qa' => ['border' => 'border-cyan-300', 'bg' => 'bg-primary-50', 'icon' => 'bg-primary-500', 'badge' => 'bg-cyan-100 text-cyan-700'],
                        ];
                        $colors = $roleColors[$role->slug] ?? ['border' => 'border-slate-300', 'bg' => 'bg-slate-50', 'icon' => 'bg-gray-400', 'badge' => 'bg-slate-100 text-slate-700'];
                    @endphp
                    ...
                    <label class="role-card relative block cursor-pointer" :class="form.role_id == {{ $role->id }} ? 'selected ring-2 ring-indigo-500 {{ $colors['border'] }} {{ $colors['bg'] }}' : 'border-slate-200 hover:border-slate-300 bg-white'">
                        ...
                        <div class="p-5 border-2 rounded-xl" :class="form.role_id == {{ $role->id }} ? '{{ $colors['border'] }} {{ $colors['bg'] }}' : 'border-slate-200 hover:border-slate-300'">
                            <div class="flex items-start gap-3">
                                <div class="w-12 h-12 {{ $colors['icon'] }} rounded-xl flex items-center justify-center shadow-lg flex-shrink-0">
                                    @if($role->slug === 'owner')
                ...
                <div class="flex justify-between pt-6 border-t border-slate-100 mt-6">
                    <x-ui.button @click="prevStep()" variant="ghost">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                        {{ __('Back') }}
                    </x-ui.button>
                    <x-ui.button @click="nextStep()" x-bind:disabled="!form.role_id" variant="primary">
                        {{ __('Continue') }}
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                        </svg>
        ...
        <!-- Step 3: Confirm & Send -->
        <div x-show="step === 3" x-cloak    class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-5 border-b border-slate-100 bg-white">
                <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2">
                    <div class="w-8 h-8 bg-secondary-500 rounded-lg flex items-center justify-center">
                        <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6 space-y-6">
                <!-- Summary Card -->
                <div class="bg-gray-50 rounded-xl p-6 border border-indigo-100">
                ...
                <div class="flex justify-between pt-4 border-t border-slate-100">
                    ...
                    <div class="flex items-center gap-3">
                        <x-ui.button as="a" href="{{ route('mindova.team.index') }}" variant="ghost">
                            {{ __('Cancel') }}
                        </x-ui.button>
