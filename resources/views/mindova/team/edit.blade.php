...
<style>
    ...
    .animate-slide-up-delay-3 { animation: slideUp 0.5s 0.3s forwards; opacity: 0; }
</style>

<div class="max-w-6xl mx-auto" x-data="editMemberForm()">
    <!-- Premium Header with Gradient -->
    <div class="relative overflow-hidden bg-primary-500 rounded-3xl p-8 mb-8">
        ...
        <div class="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="flex items-center gap-6">
                <a href="{{ route('mindova.team.index') }}" class="p-3 bg-white/10 hover:bg-white/20 rounded-xl text-white">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                </a>
                <div class="w-20 h-20 {{ $member->isOwner() ? 'bg-secondary-500' : ($member->isAdmin() ? 'bg-secondary-500' : 'bg-primary-500') }} rounded-2xl flex items-center justify-center font-bold text-white text-2xl shadow-xl ring-4 ring-white/20">
            ...
            <div class="flex items-center gap-3">
                @if($member->id !== $currentMember->id)
                    ...
                    <form action="{{ route('mindova.team.deactivate', $member) }}" method="POST" class="inline">
                        @csrf
                        <x-ui.button as="submit" onclick="return confirm('{{ __('Are you sure you want to deactivate this member?') }}')"
                            variant="ghost" class="px-4 py-2.5 bg-red-500/20 text-red-300 hover:bg-red-500/30 font-medium rounded-xl flex items-center gap-2">
                    ...
                    <form action="{{ route('mindova.team.activate', $member) }}" method="POST" class="inline">
                        @csrf
                        <x-ui.button as="submit" variant="ghost" class="px-4 py-2.5 bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 font-medium rounded-xl flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', $member->name) }}" required
                                class="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg @error('name') border-red-500 ring-2 ring-red-200 @enderror"
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                <label class="flex items-center p-4 border-2 rounded-xl cursor-pointer
                                    ...
                                    <div class="flex-1 flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-xl flex items-center justify-center
                                            {{ $role->slug === 'owner' ? 'bg-secondary-300' :
                                               ($role->slug === 'admin' ? 'bg-secondary-500' :
                                               ($role->slug === 'accounting' ? 'bg-secondary-500' :
                                               ($role->slug === 'support' ? 'bg-primary-500' : 'bg-secondary-500'))) }}">
                                            @if($role->slug === 'owner')
                        ...
                        <div>
                            ...
                            <div class="p-4 border-2 border-slate-200 rounded-xl bg-slate-50">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-xl flex items-center justify-center {{ $member->isOwner() ? 'bg-secondary-400' : ($member->isAdmin() ? 'bg-secondary-500' : 'bg-primary-500') }}">
                                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
                                        </svg>
                                    </div>
            ...
            <div class="bg-red-50 rounded-2xl border border-red-200 overflow-hidden-delay-2">
        ...
        <div class="space-y-6">
            ...
            <!-- Permissions Preview -->
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden-delay-2">
                <div class="px-6 py-5 border-b border-slate-200 bg-white">
                    <h3 class="font-bold text-slate-800 flex items-center gap-2">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden-delay-3">
                <div class="px-6 py-5 border-b border-slate-200 bg-white">
                    <h3 class="font-bold text-slate-800 flex items-center gap-2">
                ...
                <div class="p-6 space-y-3">
                    <form action="{{ route('mindova.team.resend-invitation', $member) }}" method="POST">
                        @csrf
                        <x-ui.button as="submit" variant="secondary" fullWidth class="px-4 py-3 bg-blue-50 text-blue-700 font-medium rounded-xl hover:bg-blue-100 flex items-center justify-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                            </svg>
