...
with open('new_translation_keys.txt', 'w', encoding='utf-8') as f:
    f.write(f'Total unique translation keys found: {len(all_keys)}\n')
    f.write(f'Already translated in ar.json: {len(existing_keys & all_keys)}\n')
    f.write(f'New keys needing translation: {len(new_keys)}\n\n')
    f.write('='*80 + '\n')
    f.write('NEW TRANSLATION KEYS THAT NEED ARABIC TRANSLATIONS\n')
    f.write('='*80 + '\n\n')
...
print('\nReport saved to: new_translation_keys.txt')
